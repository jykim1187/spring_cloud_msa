...
export default{
    ...
    ,data(){
        return{
            // 예시{1:true, 2:false 등등} 여기서 번호는 product.id
            selected:{},
            productList:[],
            pageSize: 7,
            currentPage: 0,
            ...
            searchType:"optional",
            searchValue:"",
            searchOptions:[{text:"선택",value:"optional"},{text:"상품명",value:"name"},{text:"카테코리",value:"category"}],


        }
    },
    created(){//로직이 많아지면 보통 크리에이티드에다 다 쓰지 않고 함수를 정의한다음 그 함수를 호출하는 식으로 진행한다.
        this.loadData();
        window.addEventListener('scroll', this.scrollPagenation)
    },
    methods:{
        searchProducts(){
            // 검색을 위해 리스트,페이지, 플래그 변수들 초기화
            this.productList = [];
            this.currentPage = 0;
        ...
        async loadData(){
            try{
                this.isLoading = true;
          // {params}는 {}라고 해서 json객체가 아니다! params 키워드 자체가 파라미터 형식(?size=xxx&page=yyy형태의 파라미터 형식으로 전송된다)
                let params ={
                    size: this.pageSize,
                ...
                const addtionalData = response.data.content.map(p=>({...p, productCount:0}))
        ...
        scrollPagenation(){
        //    화면높이 +스크롤로 이동한 거리>전체 화면 -n(원하는 길이)가 성립되면 추가 데이터 로드
       ...
       async createOrder(){
            ...
            //예시 [{productId:1, productCount:3},{productId:1, productCount:3}]이렇게 담겨야함
            const orderProductList = Object.keys(this.selected).filter(key=>this.selected[key]===true)
                                        .map(key =>{
                                            const product = this.productList.filter(p=>p.productCount>0).find(p=>p.id==key)
                                            if(product){
